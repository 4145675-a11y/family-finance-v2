...
| Health | `/api/health` | `200` רק כשהתצורה מלאה, המסד נגיש, הסכמה תואמת **והמפתח מתקבל** (`credentials: accepted`); `503 misconfigured` עם **שמות** ההגדרות כשהתצורה פגומה | `health.test.ts` 7, `check:fail-closed` 22/22, ריצה חיה |
| סירוב (`ADR-0034`) | `proxy.ts` + `lib/config/startup.ts` | תצורה פגומה → התהליך רץ, **כל** בקשה עונה `503` טקסט, שום דבר מהאפליקציה אינו רץ; הלוג מכריז `refusing to serve` בשמות | `check:fail-closed` 22/22: `/`, `/accounts`, `/api/export/backup.json`, `/auth/callback`, נתיב לא קיים — כולם 503 |
