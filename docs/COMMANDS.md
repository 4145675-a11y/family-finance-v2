...
| 19b | fail-closed gate | `npm run check:fail-closed` → `node tools/check-fail-closed.mjs` | מריץ את ה־build האמיתי ומוודא שתצורת ייצור שגויה (חנות מקומית, בלי URL, בלי origin) **אינה מגישה דבר**: כל בקשה 503, `/api/health` → `503 misconfigured` בשמות, הלוג מכריז — ושתצורה תקינה כן מגישה. 22 בדיקות (`ADR-0034`) | 9/HPO | **Active** |
