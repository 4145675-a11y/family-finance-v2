...
9. **ביטול יבוא אינו מוחק.** ה־`reverseBatch` המקומי מחק snapshots ואירועי חוב שהיבוא יצר — בניגוד ל־"כספים לא נמחקים; void/correction". מעכשיו, בשני ה־backends, ביטול יבוא מסמן אותם `voidedAt` (כפי שתנועה מסומנת `void`) והמנוע אינו סופר אותם. זהו תיקון התנהגות מכוון, מצומצם ומתועד — לא הרחבת הרשאה.
