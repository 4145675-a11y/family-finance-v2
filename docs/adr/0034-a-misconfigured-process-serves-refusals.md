...
- `check:fail-closed` נכתב מחדש: 27 בדיקות. ארבעת מקרי הסירוב מוכיחים שהתהליך **רץ**, ש־health הוא `503 misconfigured` בשם ההגדרה, שכל נתיב אפליקטיבי (`/`, `/accounts`, `/api/export/backup.json`, `/auth/callback`, נתיב לא קיים) עונה 503 טקסט ללא markup, שהלוג מכריז בשם, ושאף ערך לא הודפס — כולל ערך שהודבק בטעות למשבצת ה־origin (מקרה D). המקרה התקין מוכיח שהאפליקציה עונה (לא 503).
- **בעיה נוקבת בשם המשתנה ולעולם לא בערכו** — לא גולמי ולא מפוענח. הבעיות מגיעות ללוג, לגוף `/api/health` הציבורי ולהודעת השגיאה הנזרקת; מפתח שהודבק למשבצת הלא־נכונה (origin, backend) לא ייסע איתן. עד התיקון הזה חמש הודעות שיקפו את הערך (`deployment.test.ts` 12 בדיקות, `health.test.ts`, `startup.test.ts`).
