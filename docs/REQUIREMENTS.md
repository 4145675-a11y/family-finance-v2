...
| `PROD-FAILCLOSED-001` | build של ייצור שמוגדר לחנות המקומית אינו מגיש את האפליקציה: כל בקשה 503, health `misconfigured`; אין דגל שפותח את הדלת | 05-ARCHITECTURE-DATA.md § גבולות | 9 | Verified (M9) — ADR-0031; **המנגנון תוקן ב־ADR-0034 (2026-09-15)**: לא exit אלא סירוב לכל בקשה; בעיה נוקבת בשם המשתנה ולעולם לא בערכו (גולמי או מפוענח); `check:fail-closed` 27/27 |
...
| `IMP-DEBTMAP-001` | רשימת חובות שכותרותיה חסרות משמעות מזוהה לפי הערכים — עמודת שמות ליד עמודת סכומים — רק כשהכותרות לא נתנו תפקיד לאף עמודה, ורק כשזו העדות החזקה יותר; המיפוי מוצג לאישור, ושורה שלא יובאה מוצגת עם הסיבה; יתרה אפס, שורת סיכום וכותרת חוזרת אינן חוב פעיל | 05-ARCHITECTURE-DATA.md § Imports | DEBT-CAL | **Verified (DEBT-CAL, 2026-09-23)** — `debt-table.test.ts` 28, `debt-workbook.test.ts` 21 מקצה לקצה על xlsx אמיתי; מסמך שהמילים זיהו שומר את סוגו |
| `IMP-DEBTLEDGER-001` | יבוא חוב מאושר יוצר מלווה, חוב ואירוע `opening_balance` בתאריך היבוא; היתרה נגזרת מהאירועים בלבד; מלווה שכבר קיים דורש הכרעה מפורשת — צירוף לחוב קיים או הוצאת השורה — ולעולם לא כפילות שקטה; פעולות הכרטיס: יתרת פתיחה, הלוואה נוספת, תשלום, תיקון, מחיקה, הערה | 02-FINANCIAL-RULES.md § אינווריאנטים | DEBT-CAL | **Verified (DEBT-CAL, 2026-09-23)** — `debt-import.test.ts` 14: יתרה מ־`replayDebtBalances`, אירוע פתיחה יחיד, `needs_lender_decision` חוסם, צירוף מוסיף `new_principal` בלי כרטיס שני |
| `CAL-DUAL-001` | מועד פירעון נשמר מובְנה: תאריך לועזי קנוני, יום/חודש/שנה עבריים, ציון איזה לוח נכתב, והטקסט המקורי; שני הלוחות מוצגים יחד; המרה דרך ספרייה בוגרת (`@hebcal/core`) ולא במימוש עצמי; שנה מעוברת ואדר א׳/ב׳ נכונים; תאריך מהוסס, אדר סתמי בשנה מעוברת, יום שאינו קיים בחודש או טקסט בלתי קריא — נשמרים כסיבה לבדיקה ולא כתאריך; חזרה שנתית עברית שואלת כשיש אי־בהירות | 02-FINANCIAL-RULES.md § מוסכמות | DEBT-CAL | **Verified (DEBT-CAL, 2026-09-23)** — `hebrew-calendar` 69 בדיקות: ז׳ טבת תשפ״ז = 17/12/2026, ל׳ כסלו תשפ״ד נדחה, טבלת חודשים מדויקת (`חוב`/`תשלום`/`בנק` אינם חודשים), חזרה מחזירה `needs_choice` |
| `IMP-CLASSIFY-001` | כל שורת בנק מיובאת מגיעה עם הצעה דטרמיניסטית: סוג, קטגוריה, צד נגדי, ביטחון (high/medium/low), מזהה הכלל שיצר אותה והסבר אחד בעברית; הסיווג אינו נוגע בסכום, בכיוון או בתאריך, ואינו נשלח לשום שירות חיצוני; `high` בלבד מותר לסימון מראש | 05-ARCHITECTURE-DATA.md § Imports | TXN-INT | **Verified (TXN-INT, 2026-09-23)** — `classify.test.ts` 41 + `normalise.test.ts` 28: "ע. מסלול מורחב" בארבע כתיבים → עמלת בנק, "חיוב הלוואה" → פירעון, שורה בלתי מזוהה → low |
| `IMP-LOANLINK-001` | שורה שנקראה כפירעון הלוואה ואין בה זיהוי ודאי של המלווה חוסמת את אישור ה־batch (`needs_debt`) עד שאדם בוחר; הדגל נפרד מהביטחון — פירעון ודאי בלי מלווה עדיין אינו בטוח; מלווה אחד בדיוק שהשורה נוקבת בשמו מוצע, אפס או שניים נשאלים | 02-FINANCIAL-RULES.md § אינווריאנטים | TXN-INT | **Verified (TXN-INT, 2026-09-23)** — `classification-import.test.ts` 19: אישור נזרק, יתרות זהות לפני ואחרי, סריקת "שורות ברורות" אינה סוחפת שורת הלוואה |
| `IMP-RULES-001` | תיקון של המשפחה **מציע** להפוך לכלל ואינו נלקח אוטומטית; הכלל נבנה מהמילים הקבועות בלבד; ניתן לצפות, לשנות, לכבות ולמחוק; מבודד למשק בית אחד — במסמך ובטבלה עם RLS; כלל מזוהה עדיין עובר אישור ואינו מזיז יתרה | 07-SECURITY-PRIVACY.md § Household data | TXN-INT | **Verified (TXN-INT, 2026-09-23)** — `learn.test.ts` 13 + בידוד ו־round-trip ב־`supabase-store.test.ts`; `learned_rules` RLS enabled+forced+4 policies (34 טבלאות) |
